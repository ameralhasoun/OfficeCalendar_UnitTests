...
namespace Tests
{
    ...
    public class MessageServiceTests
    {
        ...
        [TestMethod]
        public async Task GetMessagesByUserId_ShouldReturnMessagesForSenderAndReceiver()
        {
            // Arrange
        ...
        [TestMethod]
        public async Task CreateMessage_ShouldSucceed_WhenAllFieldsValid()
        {
            // Arrange
            var msg = new Message();
            ...
            // Act
            bool ok = await _service.CreateMessage(msg, toId: 6, currentId: 5);

            // Assert – dezelfde asserts, maar licht herschreven
            Assert.IsTrue(ok);
            Assert.AreEqual(4, _context.Message.Count());
            Assert.AreEqual("Test message", _context.Message.Last().Content);
        }
